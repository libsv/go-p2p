...
type NetworkHerald struct {
	pm *PeerManager
}

func NewHerald(m *PeerManager) *NetworkHerald {
	return &NetworkHerald{pm: m}
}

func (h *NetworkHerald) CountConnectedPeers() uint {
...
func (h *NetworkHerald) AnnounceTransactions(txHashes []*chainhash.Hash, peers []PeerI) []PeerI {
	// create INV messages
	const batchSize = 256
...
// it will return the peers that the transaction was actually announced to
func (h *NetworkHerald) AnnounceTransaction(txHash *chainhash.Hash, peers []PeerI) []PeerI {
...
func (h *NetworkHerald) RequestTransactions(txHashes []*chainhash.Hash) PeerI {
...
// it will return the peer that the message was actually sent or nil if now peers are connected
func (h *NetworkHerald) RequestTransaction(txHash *chainhash.Hash) PeerI {
...
func (h *NetworkHerald) AnnounceBlock(blockHash *chainhash.Hash, peers []PeerI) []PeerI {
	// create INV message
...
// it will return the peer that the message was actually sent or nil if now peers are connected
func (h *NetworkHerald) RequestBlock(blockHash *chainhash.Hash) PeerI {
